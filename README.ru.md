...
**AmneziaWG 2.0 (UDP/443) + VLESS·REALITY (TCP/443) одной командой — с маскировкой протокола, спроектированной похожей на обычный QUIC/TLS‑трафик на порту 443.**
...
> [!IMPORTANT]
> **Без приукрашивания:** root.vpn **спроектирован** напоминать обычный QUIC/TLS‑трафик и **протестирован end‑to‑end на реальном сервере** (см. ниже). Он **не** тестировался против живых систем цензуры — маскировка это *свойство дизайна*, а не доказанный полевой результат. См. [Russia / TSPU edition](#-russia--tspu-edition) и [Честные ограничения](#️-честные-ограничения). Никакого «снейк‑ойла».
...
- 🥷 **Сделан сливаться, а не только шифровать.** Голые WireGuard/OpenVPN легко фингерпринтятся и блокируются во многих ограничительных сетях. root.vpn маскирует *первый пакет* под настоящий **QUIC client Initial к легитимному сайту**, а TCP‑нога использует **REALITY**, который реле́ит TLS‑хендшейк реального стороннего сайта — активный пробер, ткнувший в ваш сервер, получает этот реальный сайт.
...
> [!IMPORTANT]
> Конфигурационный профиль под условия ТСПУ, собранный из полевых отчётов сообщества 2026 (net4people/bbs, Habr) — **этим проектом против живого ТСПУ не проверялся**. Всё ниже повышает стоимость детекта / меняет паттерн трафика; ничто не является доказанным обходом. Решает обычно **репутация exit‑IP/ASN**, которую ни одна ручка тут не меняет.

**Если маршрут заблокировали — идите по списку** (каждый пункт — одна переменная в `defaults.conf` или env, затем `sudo awg2`):

1. **Убрать Vision → XHTTP.** Сообщаемый ноя‑2025 «фриз» ТСПУ бьёт по единому потоку `xtls‑rprx‑vision` на :443 (сервер→клиент встаёт после ~16 КБ). `TCP_TRANSPORT="xhttp"` → перезапуск. (Замена mux, которого root.vpn не везёт.)
2. **Уйти с :443.** Фриз сообщают как привязанный к порту 443; высокие порты проходили лучше. `TCP_PORT="8443"` (фаервол откроет). Минус: теряется прикрытие «похоже на HTTPS», хуже на whitelist‑операторах.
3. **Свежий хороший REALITY‑decoy + ротация.** ТСПУ считает хендшейки по SNI — общий SNI горит быстрее. `awg2 rotate-reality-target <чистый TLS1.3+h2 хост>`, затем `awg2 rotate-reality`.
4. **Соты → мобильный пресет (UDP‑нога).** `AWG_PRESET="mobile"` (Jc=3, узкий Jmax); держите `AWG_MIMICRY="quic"` + низкопрофильный `AWG_SNI`; `awg2 rotate-sni <домен>` когда оператор адаптируется.
5. **uTLS‑отпечаток Firefox.** Полевые отчёты лояльнее к Firefox/Edge, чем к Chrome: `XRAY_FP="firefox"`. (Объёмный фриз это не лечит.)
6. **Чистый ASN — то, что ручкой не решить** (см. ниже). Единственный внутренний «отбеливатель» — CDN‑фронт (`CDN_DOMAIN` + ваш домен/серт; экспериментально/вручную).
7. **Свежий Xray на обоих концах** (запиненный `XRAY_VERSION` ок; держите клиентов в той же версии).

| Симптом | Действие |
|---|---|
| UDP / QUIC режут | держите AWG с низкопрофильным SNI; если UDP неюзабелен — TCP‑нога; `AWG_PORT` на низкий UDP‑порт только если режут высокий UDP, но не :443 |
| Vision встал на :443 (~16 КБ, отходит ~60с) | `TCP_TRANSPORT="xhttp"` **и** `TCP_PORT="8443"`; смена `XRAY_FP` одна не поможет |
| Хендшейк прошёл, потом обрыв | рез ASN/IP — `rotate-reality-target` / `rotate-reality`; если повторяется — exit сожжён → чище бокс или `CDN_DOMAIN` |
| Особенности соты / allowlist‑режим | `AWG_PRESET="mobile"`, ротация SNI; на allowlist‑операторах выбор протокола не спасает — только whitelisted внутри‑РФ цепочка или CDN‑edge, а уход с :443 **навредит** |

**Не «из коробки» (вручную / roadmap):** per‑connection mux/XMUX (ручки нет — заменяет XHTTP); `XHTTP_MODE=packet-up` (правка `/etc/rootvpn/xray/params`); Shadowsocks‑2022 / AnyTLS (нет в репо); **доменная цепочка** (клиент → whitelisted внутри‑РФ VPS → зарубежный exit — сильнейший «выживатель» в РФ, но вне рамок: поднимите второй инстанс и свяжите вручную); пер‑операторные правки AWG `I1` (ручная правка `awg0.conf`).

**Реальность IP / ASN (то, что конфиг не лечит):** ТСПУ фильтрует по CIDR/ASN назначения — идеальный REALITY на «сожжённой» подсети (Hetzner AS24940, OVH, DO) всё равно умрёт после хендшейка. Грубо по выживаемости, лучше→хуже: **резидентные / мобильные (4G/LTE) > whitelisted внутри‑страны диапазоны (через цепочку) > крупные гиперскейлеры (AWS/Azure/GCP) > бюджетные DC (ждите handshake‑then‑stall)**. Выбирайте exit‑IP соответственно и считайте это движущейся мишенью.
